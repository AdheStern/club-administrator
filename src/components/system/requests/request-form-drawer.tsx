...
export function RequestFormDrawer({
  ...
  const selectedEventTableMap =
    (
      activeEvents.find((e) => e.id === selectedEventId) as
        | (EventWithRelations & { tableMap?: string })
        | undefined
    )?.tableMap ?? null;
    ...
    <>
      <Dialog open={mapDialogOpen} onOpenChange={setMapDialogOpen}>
        <DialogContent className="max-w-3xl">
          ...
          {selectedEventTableMap && (
            <div className="relative w-full aspect-[4/3] rounded-lg overflow-hidden">
              <Image
                src={selectedEventTableMap}
                alt="Mapa de mesas del evento"
                fill
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormDescription>
                                  {filteredTables.length} mesa(s) disponible(s)
                                  en {selectedSector?.name}
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                            )}
